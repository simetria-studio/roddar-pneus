...
class _CadastroPedidoState extends State<CadastroPedido> {
    ...
    try {
      ...
      if (response.statusCode == 200) {
        ...
        try {
          ...
          // Fallback tolerante para JSON levemente malformado
      ...
      if (ch == '{') {
        ...
        if (brace == 0 && start >= 0) {
          ...
          try {
            ...
            // Ignora itens irrecuperáveis, mas registra para debug
  ...
  String? _extractDataArray(String body) {
    ...
    for (int i = openBracket; i < body.length; i++) {
      ...
      if (ch == ']') {
        depth--;
        if (depth == 0) {
          return body.substring(openBracket + 1, i); // sem colchetes
        }
      }
    }
    return null;
  ...
  // Corrige problemas comuns observados no backend (aspas ausentes antes de ," e campos vazios quebrados)
